...
function(grpcmaker src_proto outputlist outputdir)
    # Generate both protobuf .pb.cc and gRPC .grpc.pb.cc from a .proto file
    get_filename_component(src_file ${src_proto} NAME_WE)
    get_filename_component(src_path ${src_proto} PATH)

    set(pb_out_cc ${outputdir}/${src_path}/${src_file}.pb.cc)
    set(pb_out_h  ${outputdir}/${src_path}/${src_file}.pb.h)
    set(grpc_out_cc ${outputdir}/${src_path}/${src_file}.grpc.pb.cc)
    set(grpc_out_h  ${outputdir}/${src_path}/${src_file}.grpc.pb.h)

    set(${outputlist} ${${outputlist}} ${pb_out_cc} ${grpc_out_cc} PARENT_SCOPE)

    add_custom_command(
        OUTPUT ${pb_out_cc} ${pb_out_h} ${grpc_out_cc} ${grpc_out_h}
        COMMAND protoc
            --cpp_out=${outputdir}
            --grpc_out=${outputdir}
            --plugin=protoc-gen-grpc=$ENV{GRPC_CPP_PLUGIN}
            -I${CMAKE_CURRENT_SOURCE_DIR}
            ${CMAKE_CURRENT_SOURCE_DIR}/${src_proto}
        DEPENDS ${CMAKE_CURRENT_SOURCE_DIR}/${src_proto}
        COMMENT "Generating gRPC code from ${src_proto}"
    )
    set_source_files_properties(
        ${pb_out_cc} ${pb_out_h}
        ${grpc_out_cc} ${grpc_out_h}
        PROPERTIES GENERATED TRUE
    )
endfunction()

function(zero_add_executable targetname srcs depends libs)
