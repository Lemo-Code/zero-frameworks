...
Zero 是一个基于 **epoll + M:N 非对称协程** 的 C++14 服务器端框架，提供日志库、协程网络库、HTTP/WebSocket 服务、Redis 协议兼容的 KV 存储引擎、RPC 框架、Sentinel 高可用集群、安全模块等开箱即用的基础设施。DB/ORM 已独立为 [`zero-orm`](/home/wangmaosen/project/kv/orm/) 模块。
...
> 测试环境：ARM64 (Kunpeng 920) / 4 线程 / Linux 6.8 / GCC 13.3 / Release 编译  
> bench2 压测工具：500 连接 × 10 秒，零错误

### TCP Echo

| 负载 | QPS | 吞吐量 |
|------|-----|--------|
| 64 B | **462,435 req/s** | 474 Mbps |
| 256 B | **439,474 req/s** | 1,800 Mbps |
| 1,024 B | **434,758 req/s** | 7,123 Mbps |

### HTTP Echo (POST)

| 负载 | QPS | 吞吐量 |
|------|-----|--------|
| 64 B | **297,103 req/s** | 599 Mbps |
| 256 B | **316,129 req/s** | 1,611 Mbps |
| 1,024 B | **362,152 req/s** | 6,299 Mbps |

### KV (Redis 协议)

| 命令 | QPS | 说明 |
|------|-----|------|
| PING | **422,675 rps** | 连接级心跳 |
| GET | **418,372 rps** | 单 key 读取 |
| SET | **261,070 rps** | 单 key 写入 |
| GET Pipeline×10 | **695,742 rps** | 管道批量读 |
| GET Pipeline×50 | **1,373,021 rps** | 大批量管道读 |

> Pipeline 批量模式可带来 **3~5 倍**吞吐提升。所有测试 0 errors。
...
# KV 服务 + AOF 持久化 + HTTP 管理端口
./bin/kv_server -p 6379 -w 4 --aof -http 8080

# KV 服务 + RPC 端口（供 Sentinel 监控）
./bin/kv_server -p 6379 -w 4 --rpc 50051
...
./bin/sentinel_server -p 6379 --rpc 50051 --sentinel \
    --sentinel-id node1 --peers 127.0.0.1:50052,127.0.0.1:50053 \
...
**当前基线：** 130+ 条 KV 命令，48 套测试全部通过（0 错误），bench2 全协议 QPS 压测工具，TCP Echo 46 万 QPS，KV GET Pipeline 137 万 rps。DB/ORM 已独立为 [`zero-orm`](/home/wangmaosen/project/kv/orm/) 模块。
