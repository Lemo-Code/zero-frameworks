...
> 基于 epoll + M:N 非对称协程的 C++14 服务器端框架，提供网络库、HTTP/WebSocket 服务、KV 存储引擎、RPC 框架、Sentinel 高可用集群等开箱即用的基础设施。

Zero 的协程调度受 [Sylar](https://github.com/sylar-yin/sylar) 启发，在协程栈分配策略、I/O 超时机制、调度器本地队列、连接亲和性等方面进行了重构与深度优化。KV 存储引擎兼容 Redis 序列化协议（RESP），标准 Redis 客户端可直接接入。

### 性能总览

> 测试环境：ARM64 (Kunpeng 920) / Linux 6.8 / GCC 13.3 / Release 编译
> bench2 压测：500 连接 × 15 秒 / 64B payload / 零错误

| 协议 | 峰值 QPS | 最优线程数 |
|------|----------|------------|
| TCP Echo | **618,556 req/s** | 4 |
| HTTP Echo | **439,692 req/s** | 8 |
| KV GET | **382,838 rps** | 6 |
| KV SET | **300,590 rps** | 8 |
...
> TCP Echo 在 4 线程达到峰值（~62 万 QPS）；HTTP Echo 线性缩放至 8 线程（~44 万 QPS）；KV GET 在 4-6 线程最优（~38 万 rps），KV SET 持续受益于更多线程（8 线程 ~30 万 rps）。推荐生产环境使用 **4-6 worker 线程**。
...
> 全部测试零错误。GET 吞吐在 6 线程达到峰值（~38 万 rps），SET 可随线程数线性扩展至 8 线程。
...
Zero 日志库提供 10 级日志级别、16 种 Appender、30 余种格式化器、5 种过滤器，支持同步/异步双模式与过载降级保护。
...
Zero KV 是构建于协程框架之上的 **Redis 协议兼容** 内存数据库，标准 Redis 客户端（`redis-cli`、Jedis、go-redis、StackExchange.Redis 等）均可接入使用。
...
Zero KV 聚焦单机高性能场景，在核心命令、Lua 脚本、主从复制、持久化、Sentinel 高可用等方面与 Redis 保持一致；在集群分片、ACL 多用户、Bitmap/HyperLogLog/Geo 等扩展数据类型上暂未覆盖。
...
基于 **TCP + 4 字节大端长度前缀 + Protobuf** 的 RPC 协议，提供连接池、负载均衡、熔断器、服务发现桥接等特性。
...
**设计特点：**
- 客户端使用原生系统调用（`connect_f`/`send_f`/`recv_f`），不依赖协程 hook，可在任意线程调用
...
基于类 Raft 共识算法的分布式哨兵机制，提供自动故障检测与主从切换，支持 Leader 选举、心跳维护与故障转移回调。
...
MySQL ORM 框架，提供模型映射、Repository 模式、查询构建器、事务管理、数据库迁移、读写分离、分库分表路由与查询缓存等能力。
...
`bench2/` 目录提供独立的 QPS 并发测试工具。所有客户端基于原生 socket I/O 实现，不依赖框架代码，可用于模拟真实场景下的压力测试。
...
`main/` 目录提供 Spring Boot 风格的生产级服务器启动入口，支持 YAML 配置加载、命令行与环境变量覆盖、Profile 激活、配置热重载、优雅关闭等特性。
...
**当前基线：** 130+ 条 KV 命令，48 套测试全部通过，bench2 全协议 QPS 压测工具，Spring Boot 风格 `main/` 启动入口。TCP Echo 62 万 QPS（4 线程），HTTP Echo 44 万 QPS（8 线程），KV GET 38 万 rps（6 线程）。DB/ORM 已独立为 [`zero-orm`](/home/wangmaosen/project/kv/orm/) 模块。
