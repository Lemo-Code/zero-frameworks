...
static zero::ConfigVar<int>::ptr g_tcp_connect_timeout =
    zero::Config::Lookup("tcp.connect.timeout", 5000, "tcp connect timeout");
...
struct _HookIniter {
    _HookIniter() {
        ...
        g_tcp_connect_timeout->addListener([](const int& old_value, const int& new_value){
                ZERO_LOG_INFO(g_logger) << "tcp connect timeout changed from "
...
retry:
    ...
    if(n == -1 && errno == EAGAIN) {
        ...
        // Always pass cancel_flag so that IOManager::cancelEvent /
        // cancelAll can notify us that this I/O was deliberately
        // cancelled (as opposed to a real EAGAIN).  Without this a
        // timeout-less event re-registers itself after cancellation,
        // leaking a pending event that keeps the IOManager alive forever.
        int rt = iom->addEvent(fd, (zero::IOManager::Event)(event), nullptr, to,
                               &tinfo.cancelled);
        if(ZERO_UNLIKELY(rt)) {
            ZERO_LOG_ERROR(g_logger) << hook_fun_name << " addEvent("
                << fd << ", " << event << ")";
            return -1;
        }
...
unsigned int sleep(unsigned int seconds) {
    ...
    zero::IOManager* iom = zero::IOManager::GetThis();
    iom->addTimer(seconds * 1000, std::bind((void(zero::Scheduler::*)
            (zero::Fiber::ptr, int thread))&zero::IOManager::schedule
            ,iom, fiber, -1));
    zero::Fiber::YieldToHold();
    return 0;
}
...
int usleep(useconds_t usec) {
    ...
    zero::Fiber::ptr fiber = zero::Fiber::GetThis();
    zero::IOManager* iom = zero::IOManager::GetThis();
    iom->addTimer(usec / 1000, std::bind((void(zero::Scheduler::*)
            (zero::Fiber::ptr, int thread))&zero::IOManager::schedule
            ,iom, fiber, -1));
    zero::Fiber::YieldToHold();
    return 0;
...
int nanosleep(const struct timespec *req, struct timespec *rem) {
    ...
    zero::Fiber::ptr fiber = zero::Fiber::GetThis();
    zero::IOManager* iom = zero::IOManager::GetThis();
    iom->addTimer(timeout_ms, std::bind((void(zero::Scheduler::*)
            (zero::Fiber::ptr, int thread))&zero::IOManager::schedule
            ,iom, fiber, -1));
    zero::Fiber::YieldToHold();
    return 0;
...
int connect_with_timeout(int fd, const struct sockaddr* addr, socklen_t addrlen, uint64_t timeout_ms) {
    ...
    int rt = iom->addEvent(fd, zero::IOManager::WRITE, nullptr, timeout_ms,
                           timeout_ms != (uint64_t)-1 ? &tinfo.cancelled : nullptr);
    if(rt == 0) {
        zero::Fiber::YieldToHold();
        if(tinfo.cancelled) {
            errno = tinfo.cancelled;
...
int accept(int s, struct sockaddr *addr, socklen_t *addrlen) {
    // If the listening socket was created outside of a hook-enabled context
    // (e.g. on the main thread before the IOManager is started) it won't be
    // tracked by FdManager.  do_io() then falls back to the real blocking
    // accept(2), which cannot be cancelled when the server shuts down.
    // Register it now so the cancellable epoll path is used instead.
    if(zero::t_hook_enable && !zero::FdMgr::GetInstance()->get(s)) {
        zero::FdMgr::GetInstance()->get(s, true);
    }
    int fd = do_io(s, accept_f, "accept", zero::IOManager::READ, SO_RCVTIMEO, addr, addrlen);
    if(fd >= 0) {
        zero::FdMgr::GetInstance()->get(fd, true);
    }
    return fd;
