...
Socket::ptr Socket::CreateTCP(zero::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(zero::Address::ptr address) {
...
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    ...
    if(rt) {
        ZERO_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
...
bool Socket::setOption(int level, int option, const void* result, socklen_t len) {
    if(setsockopt(m_sock, level, option, result, (socklen_t)len)) {
        ZERO_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
            << " level=" << level << " option=" << option
            << " errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
...
Socket::ptr Socket::accept() {
    ...
    if(newsock == -1) {
        ZERO_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno="
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if(!isValid()) {
        newSock();
        if(ZERO_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if(ZERO_UNLIKELY(addr->getFamily() != m_family)) {
        ZERO_LOG_ERROR(g_logger) << "bind sock.family("
    ...
    if(uaddr) {
        ...
        if(sock->connect(uaddr)) {
            ...
            zero::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }

    if(::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        ZERO_LOG_ERROR(g_logger) << "bind error errrno=" << errno
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if(!m_remoteAddress) {
        ZERO_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(ZERO_UNLIKELY(addr->getFamily() != m_family)) {
        ZERO_LOG_ERROR(g_logger) << "connect sock.family("
            << m_family << ") addr.family(" << addr->getFamily()
            << ") not equal, addr=" << addr->toString();
        return false;
    }
    ...
    if(timeout_ms == (uint64_t)-1) {
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            ZERO_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::listen(int backlog) {
    if(!isValid()) {
        ZERO_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if(::listen(m_sock, backlog)) {
        ZERO_LOG_ERROR(g_logger) << "listen error errno=" << errno
            << " errstr=" << strerror(errno);
        return false;
    }
    return true;
...
int Socket::send(const void* buffer, size_t length, int flags) {
    if(isConnected()) {
        return ::send(m_sock, buffer, length, flags | MSG_NOSIGNAL);
    }
    return -1;
}
...
int Socket::send(const iovec* buffers, size_t length, int flags) {
    if(isConnected()) {
        ...
        return ::sendmsg(m_sock, &msg, flags | MSG_NOSIGNAL);
    }
    return -1;
}
...
int Socket::sendTo(const void* buffer, size_t length, const Address::ptr to, int flags) {
    if(isConnected()) {
        return ::sendto(m_sock, buffer, length, flags | MSG_NOSIGNAL, to->getAddr(), to->getAddrLen());
    }
    return -1;
}
...
Address::ptr Socket::getRemoteAddress() {
    ...
    if(getpeername(m_sock, result->getAddr(), &addrlen)) {
        //ZERO_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
        //    << " errno=" << errno << " errstr=" << strerror(errno);
...
Address::ptr Socket::getLocalAddress() {
    ...
    if(getsockname(m_sock, result->getAddr(), &addrlen)) {
        ZERO_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
            << " errno=" << errno << " errstr=" << strerror(errno);
        return Address::ptr(new UnknownAddress(m_family));
    }
    if(m_family == AF_UNIX) {
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, zero::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, zero::IOManager::WRITE);
}
...
void Socket::newSock() {
    ...
    if(ZERO_LIKELY(m_sock != -1)) {
        initSock();
    } else {
        ZERO_LOG_ERROR(g_logger) << "socket(" << m_family
...
bool SSLSocket::loadCertificates(const std::string& cert_file, const std::string& key_file) {
    ...
    if(SSL_CTX_use_certificate_chain_file(m_ctx.get(), cert_file.c_str()) != 1) {
        ZERO_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file("
            << cert_file << ") error";
        return false;
    }
    if(SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
        ZERO_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file("
            << key_file << ") error";
        return false;
    }
    if(SSL_CTX_check_private_key(m_ctx.get()) != 1) {
        ZERO_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file="
...
SSLSocket::ptr SSLSocket::CreateTCP(zero::Address::ptr address) {
