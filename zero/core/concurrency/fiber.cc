...
Fiber::Fiber() {
    ...
    ZERO_LOG_DEBUG(g_logger) << "Fiber::Fiber main";
}
...
Fiber::Fiber(std::function<void()> cb, size_t stacksize, bool use_caller)
    ...
    m_stack = FiberStackAlloc(m_stacksize);
    ZERO_ASSERT(m_stack);
    if(getcontext(&m_ctx)) {
        ZERO_ASSERT2(false, "getcontext");
    }
    ...
    ZERO_LOG_DEBUG(g_logger) << "Fiber::Fiber id=" << m_id;
}

Fiber::~Fiber() {
    --s_fiber_count;
    if(m_stack) {
        ZERO_ASSERT(m_state == TERM
                || m_state == EXCEPT
                || m_state == INIT);

        FiberStackDealloc(m_stack, m_stacksize);
    } else {
        ZERO_ASSERT(!m_cb);
...
void Fiber::MainFunc() {
    ...
    try {
        ...
        cur->m_state = EXCEPT;
        ZERO_LOG_ERROR(g_logger) << "Fiber Except: " << ex.what()
            << " fiber_id=" << cur->getId()
            << std::endl
