...
public:
    ...
    void schedule(FiberOrCb fc, int thread = -1) {
        ...
        if(GetThis() == this && thread == (int)zero::GetThreadId()) {
            scheduleLocal(ft);
            return;
        }
    ...
    /**
     * @brief 按 seed 选取 io worker 线程 id（用于连接亲和调度）
     * @details use_caller 时跳过 root 线程，仅在 dedicated worker 间分配
     */
    int pickWorkerThread(uint32_t seed) const;
protected:
    /**
     * @brief 通知协程调度器有任务了
     */
...
private:
    ...
    bool scheduleNoLock(FiberAndThread& ft) {
        bool need_tickle = m_fibers.empty();
        m_fibers.push_back(std::move(ft));
        return need_tickle;
    }

    bool scheduleLocal(FiberAndThread& ft);
    bool fetchLocalTask(FiberAndThread& ft, bool& is_active);
    static std::list<FiberAndThread>& GetLocalFibers();
private:
    /// Mutex
    MutexType m_mutex;
    /// 线程池
