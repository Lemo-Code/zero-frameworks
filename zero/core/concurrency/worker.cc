...
static zero::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_worker_config
    = zero::Config::Lookup("workers", std::map<std::string, std::map<std::string, std::string> >(), "worker config");

WorkerGroup::WorkerGroup(uint32_t batch_size, zero::Scheduler* s)
...
bool WorkerManager::init(const std::map<std::string, std::map<std::string, std::string> >& v) {
    for(auto& i : v) {
        ...
        int32_t thread_num = zero::GetParamValue(i.second, "thread_num", 1);
        int32_t worker_num = zero::GetParamValue(i.second, "worker_num", 1);
