...
#ifndef __ZERO_HTTP_HTTP_H__
#define __ZERO_HTTP_HTTP_H__
...
public:
    ...
    void setPath(const char* at, size_t len) { m_path.assign(at, len);}
    ...
    void setQuery(const char* at, size_t len) { m_query.assign(at, len);}
    ...
    void setFragment(const char* at, size_t len) { m_fragment.assign(at, len);}
    ...
    void setHeader(const char* key, size_t klen, const char* val, size_t vlen);
...
public:
    ...
    void reset(uint8_t version = 0x11, bool close = true);
    ...
    void setFixedWire(const char* data, size_t len);
    bool hasFixedWire() const { return m_fixedWire && m_fixedWireLen > 0; }
    const char* fixedWireData() const { return m_fixedWire; }
    size_t fixedWireLen() const { return m_fixedWireLen; }

    static std::string buildFixedWire(uint8_t version, bool close,
                                      const std::string& body,
                                      const std::string& server = "zero/1.0.0");
private:
    /// 响应状态
    HttpStatus m_status;
    /// 版本
    ...
    const char* m_fixedWire = nullptr;
    size_t m_fixedWireLen = 0;
};
