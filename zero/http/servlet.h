...
#ifndef __ZERO_HTTP_SERVLET_H__
#define __ZERO_HTTP_SERVLET_H__
...
public:
    ...
    virtual int32_t handle(zero::http::HttpRequest::ptr request
                   ...
                   , zero::http::HttpSession::ptr session) = 0;
...
public:
    ...
    /// 函数回调类型定义
    typedef std::function<int32_t (zero::http::HttpRequest::ptr request
                   , zero::http::HttpResponse::ptr response
                   , zero::http::HttpSession::ptr session)> callback;
...
struct ServletRouteTable {
    std::unordered_map<std::string, Servlet::ptr> exact;
    std::vector<std::pair<std::string, Servlet::ptr>> globs;
    Servlet::ptr default_servlet;
};
...
private:
    ...
    /// 无锁读快照
    std::shared_ptr<ServletRouteTable> m_routeTable;
};
