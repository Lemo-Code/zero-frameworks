...
#include "zero/streams/zlib_stream.h"
...
HttpConnection::~HttpConnection() {
    ZERO_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            ZERO_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
    ...
    if(!body.empty()) {
        ...
        ZERO_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
            << " size=" << body.size();
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri
    ...
    if(!turi) {
        ZERO_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    if(!ptr) {
        ...
        if(!addr) {
            ZERO_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = m_isHttps ? SSLSocket::CreateTCP(addr) : Socket::CreateTCP(addr);
        if(!sock) {
            ZERO_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            ZERO_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= zero::GetCurrentMS())
