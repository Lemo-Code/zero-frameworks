...
public:
     ...
     */
    TcpServer(zero::IOManager* worker = zero::IOManager::GetThis()
              ,zero::IOManager* io_woker = zero::IOManager::GetThis()
     ...
     * @return 是否全部正常关闭
     */
    bool waitForConnectionsClosed(uint64_t timeoutMs);
     ...
     * @brief 返回活跃连接数
     */
    int64_t getActiveConnections() const { return m_activeConnections.load(); }
...
protected:
    ...
    std::atomic<int64_t> m_activeConnections{0};
