...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    ZERO_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    ZERO_ASSERT((window_bits >= 8 && window_bits <= 15));
    ZERO_ASSERT((memlevel >= 1 && memlevel <= 9));
...
zero::ByteArray::ptr ZlibStream::getByteArray() {
